...
/**
 * Adapter de verificación de firma de webhook Wompi (infrastructure).
 *
 * Implementa WebhookSignaturePort para Wompi usando HMAC-SHA256
 ...
 */
export class WompiWebhookSignatureAdapter implements WebhookSignaturePort {
  constructor(private readonly config: PaymentProviderConfig) {}

  /**
   * Verifica la firma del webhook Wompi.
   *
   * Wompi usa HMAC-SHA256 con la secret key sobre el raw body.
   * La firma se compara con timing-safe comparison.
   *
   * @param rawBody - Cuerpo crudo de la solicitud (string).
   * @param signature - Valor del header X-Event-Signature.
  ...
  async verify(rawBody: string, signature: string): Promise<boolean> {
    try {
      ...
      // Normalizar: Wompi puede enviar hex con o sin prefijo
